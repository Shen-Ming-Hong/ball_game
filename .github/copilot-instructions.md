...
- **Timer1 硬體中斷**: 1 秒精確計時，管理 60 秒連球視窗，配置在 `setupTimer1()` 中
- **外部中斷**: 按鈕保留但目前停用（`attachInterrupt` 已註解），備用
...
狀態轉換流程：`IDLE` →（首球進洞）→ `COUNTING` →（60 秒無進球）→ `FINISHED` →（2 秒後）→ `IDLE`

> **注意**：`MUSIC_PLAYING` 狀態目前未使用（開場音樂停用），`startCountdown()` 函數保留但未被呼叫。遊戲由首球進洞自動觸發，無需按鈕。

### 多感測器進球偵測系統

- **5 個感測器**：A1-A5（類比）+ 引腳 7-11（數位）
- **引腳模式**：`INPUT_PULLUP`，感測器斷線時保持 HIGH，防止浮空誤判
- **偵測邏輯**：`ir_digital_values[i] == 0` 表示進球（active LOW）
...
- **按鈕**: 數位引腳 2 (INT0) + GND，使用內建上拉電阻（目前停用中斷）
...
- **5 個感測器**: A1-A5（類比）+ 引腳 7-11（數位），`INPUT_PULLUP`
...
// 音效檔案：01=開場音樂(停用), 02=進球音效, 03=結束音效
```
...
const int GAME_TIME = 60;  // 連球計時視窗 60 秒
```
...
for (int i = 0; i < 5; i++) {
    ...
    // 偵測邏輯：數位輸出 == 0 時表示進球（INPUT_PULLUP，斷線=1）
}
```

### 防抖動標準

所有按鈕輸入使用 200ms 軟體防抖動窗口，結合硬體上拉電阻。IR 感測器每個獨立 1 秒暫停偵測期。

### 顯示格式慣例

- **閒置狀態**: 12 位置燈光繞圈特效 (`displayRotatingLight()`)
- **計時期間**: TM1637 顯示當前累積分數，串列輸出剩餘時間與分數
- **高分顯示**: 分數超過 9999 時顯示 "H-i" (`{0x76, 0x06, 0x00, 0x00}`)

### 變數命名模式

- **中斷相關**: 使用 `volatile` 關鍵字，如 `volatile bool timer_active`
- **引腳定義**: `_PIN` 後綴常數，如 `IR_DIGITAL_PINS[5]`
- **感測器陣列**: 複數形式，如 `ir_digital_values[5]`, `balls_detected[5]`
- **時間戳變數**: `_until` 後綴，如 `detection_pause_until[5]`
...
- 使用 `noInterrupts()`/`interrupts()` 保護關鍵區段（重置 `countdown_time` 時需要）

### IR 感測器穩定性

- 數位引腳必須使用 `INPUT_PULLUP`，避免感測器斷線時浮空拉低觸發誤判
- TCRT5000 output 為 active LOW，正常無遮擋輸出 HIGH，有遮擋（進球）輸出 LOW
...
1. 擴展 `GameState` 枚舉（當前使用：IDLE, COUNTING, FINISHED；MUSIC_PLAYING 保留未用）
...
- loop() 每 500ms 自動印出所有 IR 感測器類比值與數位值（`[IR]` 前綴）
- 關鍵變數狀態列印在主循環中，非中斷內
- 硬體問題優先檢查線路連接和電源
